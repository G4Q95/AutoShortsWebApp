...
*   **Endpoint 2.2: `PUT /projects/{project_id}`**
    ...
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (may require manually editing a title via UI if possible). Run Mock Playwright Tests.
...
*   **Endpoint 2.3: `DELETE /projects/{project_id}`**
    ...
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (delete a test project). Run Mock Playwright Tests.

*   **Endpoint 2.4: `GET /projects`**
    - [ ] **Action:** Move the `get_projects` function logic. Decorate with `@project_router.get("/projects", ...)`.
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (verify My Projects page loads). Run Mock Playwright Tests.

*   **Endpoint 2.5: `POST /projects`**
    - [ ] **Action:** Move the `create_project` function logic. Decorate with `@project_router.post("/projects", ...)`.
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (create a new project). Run Mock Playwright Tests.
...
*   **Endpoint 3.1: `POST /projects/{project_id}/scenes` (Add Scene)**
    ...
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (add a scene to a project). Run Mock Playwright Tests.
...
*   **Endpoint 3.2: `PUT /projects/{project_id}/scenes/{scene_id}` (Update Scene)**
    ...
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (edit scene text). Run Mock Playwright Tests.
...
*   **Endpoint 3.3: `DELETE /projects/{project_id}/scenes/{scene_id}` (Delete Scene)**
    ...
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (delete a scene). Run Mock Playwright Tests.
...
*   **Endpoint 3.4: `PUT /projects/{project_id}/scenes/reorder`**
    ...
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (drag/drop scenes). Run Mock Playwright Tests.
...
*   **Endpoint 3.5: `PUT /projects/{project_id}/scenes/{scene_id}/trim`**
    ...
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (adjust scene trim). Run Mock Playwright Tests.
...
- [ ] **If YES:**
    ...
    - [ ] **Test:** After each move: Restart backend. Manual check (if possible). Mock tests.
...
*   **Endpoint 5.1: `POST /projects/{project_id}/process` (Start Generation)**
    ...
    - [ ] **Test:** Restart backend. Perform Manual Browser Sanity Check (click Generate Video). Run Mock Playwright Tests.
...
*   **Endpoint 5.2: `GET /projects/{project_id}/process/{task_id}` (Get Status)**
    ...
    - [ ] **Test:** Restart backend. Manual check (verify status updates if possible). Run Mock Playwright Tests.
...
- [ ] **Action:** Review `api/projects.py` - all endpoint logic should now be moved. Add comments like `# TODO: Remove after verification` to the original functions.
...
- [ ] **Action:** Once Phase 6 testing passes consistently: Add `@router.deprecated()` decorator or clear comments indicating deprecation to the original functions in `api/projects.py`.
- [ ] **Action:** (Future) After a period of stability, physically remove the old endpoint code from `api/projects.py`.
...
*   **Testing:** Be meticulous. Do not skip tests after any step.
*   **Service Layer:** Use functions from `web/backend/app/services/` where possible.
