...
function projectReducer(state: ProjectState, action: ProjectAction): ProjectState {
  switch (action.type) {
    ...
    case 'UPDATE_SCENE_TEXT': {
      ...
      const updatedScenes = state.currentProject.scenes.map((scene) =>
        scene.id === action.payload.sceneId ? { ...scene, text: action.payload.text } : scene
      );
    ...
    case 'LOAD_PROJECT_SUCCESS': {
      ...
      console.log(
        `[ProjectReducer] Loaded project:`,
        action.payload.project
          ? `id: ${action.payload.project.id}, title: "${action.payload.project.title}"`
          : 'null'
      );
      ...
      console.log(
        `[ProjectReducer] Updated state with loaded project, currentProject set to:`,
  ...
  const loadProject = useCallback(
    async (projectId: string) => {
      ...
      try {
        ...
        console.log(
          `[ProjectProvider] Project retrieved:`,
    ...
    async (url: string) => {
      ...
      try {
        ...
        if (data.has_media && data.media_url) {
          ...
          console.log(
            'ProjectProvider: No media found in response. has_media:',
            data.has_media,
            'media_url:',
            data.media_url
          );
